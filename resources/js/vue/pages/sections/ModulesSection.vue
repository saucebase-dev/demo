...
<template>
    <section ref="sectionRef" class="relative mt-16 overflow-hidden">
        <!-- Split-screen framework picker -->
        <div
            class="relative z-0 min-h-120 w-full overflow-hidden [mask-image:linear-gradient(to_bottom,black_60%,transparent)]"
        >
            <!-- Background: two colored halves with gradient toward center -->
            <div class="absolute inset-0 grid grid-cols-2">
                <div
                    class="bg-linear-to-r from-gray-400 to-(--vue-color)/70 dark:from-slate-900"
                />
                <div
                    class="bg-linear-to-l from-gray-400 to-(--react-color)/70 dark:from-slate-900"
                />
            </div>

            <!-- Corner text: constrained to container, aligned to outer edges, anchored to bottom -->
            <div
                class="relative mx-auto grid min-h-120 max-w-6xl grid-cols-2 items-end px-6 pb-40 sm:px-10 lg:px-8"
            >
                <!-- Vue side -->
                <div
                    class="flex flex-col items-center p-10 text-center text-white lg:p-16"
                >
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24"
                        fill="currentColor"
                        class="logo-vue size-20 opacity-90"
                        aria-hidden="true"
                    >
                        <path
                    ...
                    <h3 class="mt-2 text-2xl font-bold">Vue</h3>
                    <p class="mt-1 max-w-xs text-lg leading-relaxed opacity-90">
                ...
                >
                    <svg
                        ...
                        viewBox="0 0 24 24"
                        fill="currentColor"
                        class="logo-react size-20 opacity-90"
                        aria-hidden="true"
                    >
                        <path
                    ...
                    <h3 class="mt-2 text-2xl font-bold">React</h3>
                    <p class="mt-1 max-w-xs text-lg leading-relaxed opacity-90">
            ...
            >
                <button
                    type="button"
                    class="pointer-events-auto relative flex h-14 w-28 cursor-pointer items-center rounded-full p-1.5 shadow-[inset_0_2px_6px_rgba(0,0,0,0.35)] ring-4 ring-white/70 backdrop-blur-sm transition-colors duration-300"
                    :class="
                ...
                >
                    <div
                        class="size-11 rounded-full bg-gray-100 shadow-[inset_0_3px_10px_rgba(255,255,255,1),inset_0_-2px_6px_rgba(0,0,0,0.15),0_4px_8px_rgba(0,0,0,0.3)] transition-transform duration-300"
                        :class="{
        ...
        >
            ...
            <div
                class="absolute inset-0 -z-1 overflow-hidden mask-t-from-95% mask-b-from-95% md:mask-r-from-95% md:mask-l-from-95%"
            >
                <div
                    class="pattern-light absolute -inset-10 top-80 md:rotate-[-5deg] md:skew-x-10 dark:hidden"
                />
                <div
                    class="pattern-dark absolute -inset-10 top-80 hidden md:rotate-[-5deg] md:skew-x-10 dark:block"
                />
            </div>
            ...
            >
                <ModuleCard
                    ...
                    :class="{ 'opacity-35': !mod._supported }"
                    @select="
        ...
        <div class="relative z-10 my-8 -mt-36 mb-36 flex justify-center">
            <div class="relative inline-flex">
                <div
                    class="stripe absolute inset-0 translate-y-3 rounded-full [--mod-color:var(--foreground)]"
                />
                <a
...
.logo-vue {
    filter: drop-shadow(
        0 4px 8px color-mix(in oklch, var(--vue-color) 85%, black)
    );
}

.logo-react {
    filter: drop-shadow(
        0 4px 8px color-mix(in oklch, var(--react-color) 85%, black)
    );
}
